...
func newPulseAudioOutput(reader librespot.Float32Reader, sampleRate int, channels int, externalVolumeUpdate *RingBuffer[float32]) (*pulseAudioOutput, error) {
	...
	out.stream, err = out.client.NewPlayback(pulse.Float32Reader(out.float32Reader), pulse.PlaybackSampleRate(out.sampleRate), channelOpt)
	if err != nil {
		return nil, err
	}
	...
	volume := cvol.Avg().Linear()
	externalVolumeUpdate.Put(float32(volume))
...
func (out *pulseAudioOutput) SetVolume(vol float32) {
	volume := proto.ChannelVolumes{proto.LinearVolume(float64(vol))}
	err := out.stream.SetVolume(volume)
	if err != nil {
