...
func newPulseAudioOutput(reader librespot.Float32Reader, sampleRate int, channels int, externalVolumeUpdate chan float32) (*pulseAudioOutput, error) {
	...
	out.volume = cvol.Avg()
	sendVolumeUpdate(externalVolumeUpdate, float32(out.volume)/float32(proto.VolumeNorm))
	...
	go func() {
		for cvol := range volumeUpdates {
			...
			if volume != out.volume {
				sendVolumeUpdate(externalVolumeUpdate, float32(volume)/float32(proto.VolumeNorm))
				out.volume = volume
			}
...
func (out *pulseAudioOutput) SetVolume(vol float32) {
	volume := proto.Volume(math.Round(float64(vol) * float64(proto.VolumeNorm)))
