...
type SwitchingAudioSource struct {
	...
	//
	// While crossfading is enabled, up to crossfadeSamples samples are
	// buffered ahead of playback in a ring buffer (buf). This lookahead is
	// how the end of the primary source is discovered before it is audible:
	// when the decoder returns EOF, the ring holds exactly the track's tail.
	// If a secondary source is available, the tail is faded out while the
	// secondary fades in with equal-power gains.
	crossfadeSamples int
	buf              []float32 // lookahead ring buffer, cap == crossfadeSamples
	...
	scratch          []float32 // reusable buffer for secondary reads while fading
}
...
func (s *SwitchingAudioSource) readCrossfade(p []float32) (n int, err error) {
	for {
		...
		// Top up the lookahead buffer until it is full or the track ends.
		for !s.primaryEOF && s.bufLen < len(s.buf) {
			free := s.freeChunk()
		...
		if !s.primaryEOF {
			// Serve only the headroom above the crossfade reserve. The fill
			// loop above guarantees the buffer is full here, so this is
			// always positive.
...
func (s *SwitchingAudioSource) SetPositionMs(pos int64) error {
	...
	// The source is live again after a successful seek.
	s.eofReported = false
	return nil
}
